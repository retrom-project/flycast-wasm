...
def release(candidate, output, tag, commit):
    ...
    if not re.fullmatch(config["releaseTagPattern"], tag):
        raise ValueError("FLYCAST_RELEASE_TAG_INVALID")
    if not re.fullmatch(r"[0-9a-f]{40}", commit):
        raise ValueError("FLYCAST_RELEASE_COMMIT_INVALID")
    ...
    if (descriptor.get("dirty") is not False or descriptor.get("commit") != commit
        ...
        raise ValueError("FLYCAST_RELEASE_SOURCE_INVALID")
    ...
    if len(records) != len(expected) or {r["filename"] for r in records} != expected:
        raise ValueError("FLYCAST_RELEASE_FILES_INVALID")
    ...
    for record in records:
        ...
        if not data or len(data) != record["sizeBytes"] or hashlib.sha256(data).hexdigest() != record["sha256"]:
            raise ValueError("FLYCAST_RELEASE_INTEGRITY_INVALID")
    output.mkdir(parents=True, exist_ok=True)
    if any(output.iterdir()):
        raise ValueError("FLYCAST_RELEASE_OUTPUT_NOT_EMPTY")
...
if __name__ == "__main__":
    ...
    if subprocess.check_output(["git", "status", "--porcelain"], cwd=ROOT):
        raise ValueError("FLYCAST_RELEASE_SOURCE_DIRTY")
